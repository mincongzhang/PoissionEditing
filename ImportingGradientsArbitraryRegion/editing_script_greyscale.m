...
clear
close all
object_source = imread('shark.jpg');
background = imread('sky.jpg');
object_source = rgb2gray(object_source);
...
object_source = double(object_source);
raw_object = object_source.*roipoly_mask;
%figure,imshow(uint8(raw_object));title('raw_object');

%get input coordinate on background
figure,imshow(background);title('4. background');
